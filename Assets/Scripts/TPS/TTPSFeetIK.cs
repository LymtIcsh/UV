...
public class TTPSFeetIK : MonoBehaviour
{
    #region Feet
    ...
    private void SetOriginValues()
    {
        ...
        {
            isOrigin = false;
            Vector3 temp = transform.InverseTransformPoint(tps.at.bodyPosition);
    ...
    {
        Vector3 targetIKPosition = tps.at.GetIKPosition(foot);
        if (positionIKHolder != Vector3.zero)
        {
            ...
            tps.at.SetIKRotation(foot, rotationIKHolder);
        }
        tps.at.SetIKPosition(foot, targetIKPosition);
    }
    ...
    private void MovePelvisHeight()
    {
        ...
        {
            lastPelvisPositionY = tps.at.bodyPosition.y;
            return;
        }
        ...
        Vector3 newPelvisPosition = tps.at.bodyPosition + Vector3.up * totalOffset;

        newPelvisPosition.y = Mathf.Lerp(lastPelvisPositionY, newPelvisPosition.y, pelvisUpAndDownSpeed);
        tps.at.bodyPosition = newPelvisPosition;
        lastPelvisPositionY = tps.at.bodyPosition.y;
    }
    ...
    {
        footPosition = tps.at.GetBoneTransform(foot).position;
        footPosition.y = transform.position.y + heightFromGroundRaycast;
    }

    private void MoveHandsHeight(AvatarIKGoal hand)
    {
        Vector3 targetIKPosition = tps.at.GetIKPosition(hand);
        Vector3 targetBodyPosition = transform.InverseTransformPoint(tps.at.bodyPosition);
        tps.at.SetIKPositionWeight(hand, 1.0f);
        tps.at.SetIKPosition(hand, new Vector3(targetIKPosition.x, targetIKPosition.y - (originBodyPositionY - targetBodyPosition.y), targetIKPosition.z));
    }
}
